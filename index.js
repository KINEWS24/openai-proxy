// index.js – ThinkAI Nexus (Finale, robuste Version)
...
// --- Überarbeitete, robuste Funktion zum Initialisieren des Wissens-Index ---
    ...
    try {
        ...
        for (const file of mdFiles) {
            ...
            const cleanTextForIndexing = fileContent
                .replace(/{\s*"OwnerUserID":[\s\S]*?}/, '') // Entfernt den kompletten JSON-Block
                .replace(/\s\s+/g, ' ') // Entfernt überflüssige Leerzeichen
                .trim();

            if (cleanTextForIndexing) {
                // Metadaten für die Anzeige werden weiterhin extrahiert
                ...
                validDocuments.push({ 
                    ...
                    fullContent: fileContent, // Der volle Inhalt wird für den Chat-Kontext behalten
                    title: title,
                    url: url,
                    tags: tags
                });
